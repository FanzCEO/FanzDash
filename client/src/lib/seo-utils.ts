 ...
 */
export function generatePageTitle(pageTitle: string, includeSubtitle = true): string {
 ...
 */
export function generateKeywords(pageSpecificKeywords: string[] = []): string[] {
  const baseKeywords = [
    "creator platform", 
    "content management", 
    "live streaming", 
...
export const adminPageSEO = {
  systemConfiguration: {
    ...
    description: "Configure system-wide settings for your creator platform including security, performance, uploads, and content management options.",
    keywords: ["system settings", "platform configuration", "security settings", "performance optimization"] as string[],
    structuredData: {
      "@context": "https://schema.org",
      "@type": "WebPage",
      "name": "System Configuration",
      "description": "Enterprise system configuration panel for creator platform management",
      "mainEntity": {
  ...
  logoFaviconManagement: {
    ...
    description: "Manage your brand logos and website favicons across all devices and platforms. Upload, optimize, and configure branding elements.",
    keywords: ["logo management", "favicon upload", "branding", "website icons", "brand identity"] as string[],
    structuredData: {
      "@context": "https://schema.org",
      "@type": "WebPage",
  ...
  paymentGatewaySetup: {
    ...
    description: "Configure and manage payment processors including Stripe, PayPal, cryptocurrency, and regional payment methods with fraud protection.",
    keywords: ["payment gateway", "stripe setup", "paypal integration", "payment processing", "fraud protection"] as string[],
    structuredData: {
      "@context": "https://schema.org",
      "@type": "WebPage",
  ...
  kycVerificationSetup: {
    ...
    description: "Configure know-your-customer verification requirements, compliance settings, and identity verification workflows for regulatory compliance.",
    keywords: ["KYC verification", "identity verification", "compliance", "AML", "regulatory requirements"] as string[],
    structuredData: {
      "@context": "https://schema.org",
      "@type": "WebPage",
  ...
  seoConfiguration: {
    ...
    description: "Optimize your platform for search engines and app stores with comprehensive SEO settings, meta tags, structured data, and performance optimization.",
    keywords: ["SEO settings", "search optimization", "meta tags", "structured data", "app store optimization"] as string[],
    structuredData: {
      "@context": "https://schema.org",
      "@type": "WebPage",
      "name": "SEO Configuration Panel",
      "description": "Search engine and app store optimization configuration interface",
      "mainEntity": {
        "@type": "SoftwareApplication",
...
export function generateAdminBreadcrumbs(currentPage: string) {
  ...
  const pageBreadcrumbs: Record<string, { label: string; href: string }> = {
    "system-configuration": { label: "System Configuration", href: "/system-configuration" },
    "logo-favicon-management": { label: "Logo & Favicon Management", href: "/logo-favicon-management" },
    "payment-gateway-setup": { label: "Payment Gateway Setup", href: "/payment-gateway-setup" },
    "kyc-verification-setup": { label: "KYC Verification Setup", href: "/kyc-verification-setup" },
    "seo-configuration": { label: "SEO Configuration", href: "/seo-configuration" }
  };
 ...
 */
export function generateAdminPageStructuredData(pageType: keyof typeof adminPageSEO) {
  const baseData = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    ...
    "breadcrumb": {
      "@type": "BreadcrumbList",
      "itemListElement": generateAdminBreadcrumbs(pageType.replace(/([A-Z])/g, '-$1').toLowerCase()).map((item, index) => ({
        "@type": "ListItem",
        "position": index + 1,
        "name": item.label,
 ...
 */
function calculateSEOScore(data: PageSEOData, issues: string[], warnings: string[]): number {
  let score = 100;
