...
export function IntelligentModerationWorkflow() {
    ...
    const mockContent: ContentItem[] = [
      ...
      },
      {
        ...
        aiAnalysis: {
          ...
          flags: ['unverified-co-star', '2257-forms-missing', 'content-policy-violation'],
          confidence: 91
        },
        moderationStatus: 'requires_review',
        auditTrail: []
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'requires_review': return <AlertTriangle className="w-4 h-4 text-orange-400" />;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-cyan-400">Intelligent Moderation Workflow</h2>
        ...
        <div className="flex items-center gap-4">
          ...
          <Badge variant="outline" className="text-orange-400 border-orange-400">
            {contentQueue.filter(item => item.moderationStatus === 'requires_review').length} Action Required
          </Badge>
        </div>
      </div>
          ...
          <Card key={item.id} className="bg-black/40 border-primary/20 cyber-border" data-testid={`content-card-${item.id}`}>
            <CardHeader>
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="outline" className={`text-xs ${getComplianceColor(item.aiAnalysis?.compliance_risk || 'low')}`}>
                    {item.aiAnalysis?.compliance_risk?.toUpperCase() || 'PENDING'}
                  </Badge>
                </div>
              </div>
            </CardHeader>
            ...
            <CardContent className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="text-center p-2 rounded border border-primary/20 bg-black/30">
                      ...
                      <p className="text-sm font-bold text-white">{item.aiAnalysis.gpt5_score}%</p>
                    </div>
                    <div className="text-center p-2 rounded border border-primary/20 bg-black/30">
                      <p className="text-xs text-gray-400">Vision Score</p>
                      <p className="text-sm font-bold text-white">{item.aiAnalysis.gpt4o_vision_score}%</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="text-white">{item.aiAnalysis.confidence}%</span>
                    </div>
                    <Progress value={item.aiAnalysis.confidence} className="h-2" />
                  </div>

                  {/* Compliance Flags */}
                  {item.aiAnalysis.flags.length > 0 && (
                    <div className="space-y-2">
                      <p className="text-xs font-semibold text-yellow-400">Compliance Flags:</p>
                      <div className="space-y-1">
                        {item.aiAnalysis.flags.map((flag, index) => (
                          <div key={index} className="flex items-center gap-2 text-xs">
                            <AlertTriangle className="w-3 h-3 text-yellow-500" />
                            <span className="text-yellow-300">{flag.replace(/-/g, ' ').toUpperCase()}</span>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              {item.coStars.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-white mb-2">Co-Stars ({item.coStars.length}):</p>
                  <div className="flex flex-wrap gap-1">
                    {item.coStars.map((coStar) => (
                      <Badge key={coStar} variant="outline" className="text-xs text-cyan-400 border-cyan-400">
                        {coStar}
                      </Badge>
                    ))}
                  </div>
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  {isAnalyzing && selectedItem?.id === item.id ? 'Analyzing...' : 'Re-Analyze'}
                </Button>
                ...
                {item.moderationStatus !== 'approved' && (
                  <ComplianceGuard
                    ...
                    requiresSuperAdmin={item.aiAnalysis?.compliance_risk === 'critical'}
          ...
          <Card className="w-full max-w-md bg-black/90 border-primary/20">
            <CardHeader>
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <CardTitle className="text-cyan-400">AI Analysis in Progress</CardTitle>
                  <CardDescription>Processing content through ChatGPT-4o/GPT-5</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <Alert className="border-cyan-500/50 bg-cyan-500/10">
                ...
                <AlertDescription className="text-cyan-300 text-sm">
                  AI models are analyzing content for compliance with platform policies and legal requirements.
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
