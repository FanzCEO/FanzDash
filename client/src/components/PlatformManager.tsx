...
export function PlatformManager() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case "active": return <CheckCircle className="w-4 h-4 text-secondary" />;
      case "inactive": return <Clock className="w-4 h-4 text-muted-foreground" />;
      case "maintenance": return <Settings className="w-4 h-4 text-accent" />;
  ...
  const getNicheIcon = (niche: string) => {
    ...
    if (lowerNiche.includes("adult")) return <Lock className="w-4 h-4 text-destructive" />;
    if (lowerNiche.includes("dating")) return <Users className="w-4 h-4 text-primary" />;
    if (lowerNiche.includes("social")) return <Network className="w-4 h-4 text-secondary" />;
    if (lowerNiche.includes("gaming")) return <Zap className="w-4 h-4 text-accent" />;
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center space-x-2">
              ...
              <span className="cyber-text-glow">MULTI-PLATFORM CONTROL CENTER</span>
            </CardTitle>
            <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
              <DialogTrigger asChild>
                <Button className="neon-button" data-testid="add-platform-button">
              ...
              <DialogContent className="max-w-2xl cyber-card">
                <DialogHeader>
                  <DialogTitle className="cyber-text-glow">Add New Platform</DialogTitle>
                </DialogHeader>
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select value={newPlatform.niche} onValueChange={(value) => setNewPlatform({...newPlatform, niche: value})}>
                        <SelectTrigger className="glass-effect">
                          <SelectValue placeholder="Select niche" />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="adult_content">Adult Content</SelectItem>
                          <SelectItem value="dating_social">Dating & Social</SelectItem>
                          <SelectItem value="gaming_entertainment">Gaming & Entertainment</SelectItem>
                          <SelectItem value="social_media">Social Media</SelectItem>
                          <SelectItem value="content_creation">Content Creation</SelectItem>
                          <SelectItem value="live_streaming">Live Streaming</SelectItem>
                          <SelectItem value="marketplace">Marketplace</SelectItem>
                          <SelectItem value="community_forum">Community Forum</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    <div>
                      <label className="text-sm font-medium mb-2 block">Risk Threshold</label>
                      <Select value={newPlatform.riskThreshold.toString()} onValueChange={(value) => setNewPlatform({...newPlatform, riskThreshold: parseFloat(value)})}>
                        <SelectTrigger className="glass-effect">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setNewPlatform({...newPlatform, webhookUrl: e.target.value})}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <Switch
                        ...
                        onCheckedChange={(checked) => setNewPlatform({...newPlatform, autoBlock: checked})}
                      />
                      <label className="text-sm">Auto-block flagged content</label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Switch
                        checked={newPlatform.requireManualReview}
                        onCheckedChange={(checked) => setNewPlatform({...newPlatform, requireManualReview: checked})}
                      />
                ...
                >
                      ...
                      <div>
                        <div className="flex items-center space-x-3 mb-1">
                          ...
                          <Badge className={getStatusColor(platform.status)}>
                            ...
                            <span className="ml-1">{platform.status.toUpperCase()}</span>
                          </Badge>
                        ...
                        <div className="text-sm text-muted-foreground">
                          ...
                          {platform.domain}
                          {" • "}
                          Risk threshold: {(platform.moderationRules.riskThreshold * 100).toFixed(0)}%
                        </div>
                      </div>
                    </div>
                              ...
                              <div className="grid grid-cols-2 gap-6">
                                <div>
                                  ...
                                  <div className="space-y-2 text-sm">
                                    ...
                                    <div>API Endpoint: {selectedPlatform.apiEndpoint}</div>
                                    <div>Webhook: {selectedPlatform.webhookUrl}</div>
                                  </div>
                                </div>
                                <div>
                                  <h4 className="font-semibold mb-3">Moderation Rules</h4>
                                  <div className="space-y-2 text-sm">
                                    <div>Auto Block: {selectedPlatform.moderationRules.autoBlock ? "Enabled" : "Disabled"}</div>
                                    <div>Manual Review: {selectedPlatform.moderationRules.requireManualReview ? "Required" : "Optional"}</div>
                                    <div>Risk Threshold: {(selectedPlatform.moderationRules.riskThreshold * 100).toFixed(0)}%</div>
                                  </div>
                                </div>
                              </div>
                            </div>
                  ...
                  <div className="grid grid-cols-4 gap-4">
                    <div className="text-center p-3 bg-muted/20 rounded-lg">
                      <div className="text-lg font-bold text-primary">{platform.stats.totalContent}</div>
                      <div className="text-xs text-muted-foreground">Total Content</div>
                    </div>
                    <div className="text-center p-3 bg-muted/20 rounded-lg">
                      <div className="text-lg font-bold text-secondary">{platform.stats.dailyContent}</div>
                      <div className="text-xs text-muted-foreground">Today</div>
                    </div>
                    <div className="text-center p-3 bg-muted/20 rounded-lg">
                      <div className="text-lg font-bold text-destructive">{platform.stats.blockedContent}</div>
                      <div className="text-xs text-muted-foreground">Blocked</div>
                    </div>
                    <div className="text-center p-3 bg-muted/20 rounded-lg">
                      <div className="text-lg font-bold text-accent">{platform.stats.flaggedContent}</div>
                  ...
                  {connection && (
                    <div className="mt-4 p-3 bg-muted/10 rounded-lg">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span>Last sync: {new Date(platform.stats.lastSync).toLocaleTimeString()}</span>
                      </div>
                    </div>
                  )}
                </div>
