...
export function GiftSendingModal({
  ...
  const totalTaxes = taxRates.reduce((sum, tax) => 
    sum + (selectedGiftData ? (selectedGiftData.price * tax.percentage / 100) : 0), 0
  );
  const totalPrice = selectedGiftData ? selectedGiftData.price + totalTaxes : 0;

  const getRarityIcon = (rarity: Gift['rarity']) => {
    switch (rarity) {
      case 'common': return <Gift className="h-4 w-4 text-gray-500" />;
      case 'rare': return <Heart className="h-4 w-4 text-blue-500" />;
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold">
                      {currencySymbol}{selectedGiftData.price.toFixed(2)}
                    </span>
                  </div>
                  ...
                  {taxRates.map((tax) => (
                    <div key={tax.id} className="flex justify-between items-center text-sm">
                      ...
                      <span>
                        {currencySymbol}
                        {(selectedGiftData.price * tax.percentage / 100).toFixed(2)}
                      </span>
                    </div>
                  ))}
          ...
          <div className="flex space-x-3">
            ...
            <Button
              type="submit"
              className="flex-1"
              disabled={!selectedGift || isSubmitting || totalPrice > userWalletBalance}
