...
export default function PaymentGatewaySetup() {
  ...
  const [gateways, setGateways] = useState<PaymentGateway[]>([
    {
      ...
      config: {
        ...
        webhookUrl: 'https://api.fanzdash.com/webhooks/stripe'
      },
      status: 'active',
      lastTested: '2025-01-15T10:30:00Z',
      totalTransactions: 1245,
      totalVolume: 89750.50
    },
    {
      id: 'paypal',
      name: 'paypal',
      displayName: 'PayPal',
      description: 'Popular digital wallet and payment processor',
  ...
  const [globalSettings, setGlobalSettings] = useState({
    ...
    supportedCountries: ['US', 'CA', 'GB', 'AU', 'DE', 'FR', 'ES', 'IT', 'NL', 'SE', 'NO', 'DK'],
    blockedCountries: ['XX']
  });
  ...
  const toggleGateway = async (gatewayId: string, enabled: boolean) => {
    setGateways(prev => prev.map(gw => 
      gw.id === gatewayId 
        ? { ...gw, isEnabled: enabled, status: enabled ? 'active' : 'inactive' }
        : gw
    ));
  ...
  const GatewayCard = ({ gateway }: { gateway: PaymentGateway }) => {
    ...
    return (
      <Card className={`hover:shadow-lg transition-all duration-300 ${selectedGateway === gateway.id ? 'ring-2 ring-primary' : ''}`}>
          ...
          <div className="space-y-4">
            ...
            {gateway.totalTransactions > 0 && (
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <div className="font-semibold">{gateway.totalTransactions.toLocaleString()}</div>
                </div>
                <div>
                  <div className="text-muted-foreground">Volume</div>
                  <div className="font-semibold">${gateway.totalVolume.toLocaleString()}</div>
                </div>
              </div>
            )}
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-medium">${gateway.minAmount} - ${gateway.maxAmount.toLocaleString()}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">Test Mode</span>
                <Badge variant={gateway.isTestMode ? "outline" : "secondary"}>
                  {gateway.isTestMode ? <TestTube className="h-3 w-3 mr-1" /> : <Lock className="h-3 w-3 mr-1" />}
            ...
            <div>
              <div className="text-sm text-muted-foreground mb-2">Supported Currencies</div>
  ...
  const ConfigurationPanel = ({ gateway }: { gateway: PaymentGateway }) => (
    <Card>
      ...
      <CardContent className="space-y-6">
            ...
            <div key={key} className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  value={value}
                  onChange={(e) => updateGatewayConfig(gateway.id, key, e.target.value)}
                  placeholder={isSecret ? "Enter secret key..." : `Enter ${key}...`}
        ...
        <Alert className={gateway.isTestMode ? "border-yellow-500" : "border-green-500"}>
          {gateway.isTestMode ? (
            ...
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-blue-600">
                  {gateways.reduce((sum, gw) => sum + gw.totalTransactions, 0).toLocaleString()}
                </div>
            ...
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-purple-600">
                  ${gateways.reduce((sum, gw) => sum + gw.totalVolume, 0).toLocaleString()}
                </div>
            ...
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-orange-600">
                  {Math.round(gateways.reduce((sum, gw) => sum + gw.processingFee, 0) / gateways.length * 100) / 100}%
                </div>
                <div className="text-sm text-muted-foreground">Avg Processing Fee</div>
              </CardContent>
            </Card>
          </div>
            ...
            <CardContent>
              <div className="space-y-4">
                <Alert>
                  ...
                  <AlertDescription>
                    <strong>Recommended Setup:</strong> Enable Stripe for global card payments and PayPal for digital wallet users. This covers 95% of payment preferences worldwide.
                  </AlertDescription>
                </Alert>
          ...
          {selectedGateway ? (
            <div className="space-y-6">
              ...
              {(() => {
                ...
                return gateway ? <ConfigurationPanel gateway={gateway} /> : null;
              })()}
            </div>
          ) : (
            <div className="text-center py-16">
              <Settings className="h-16 w-16 text-muted-foreground mx-auto mb-4" />
              <h3 className="text-xl font-semibold mb-2">Select a Gateway to Configure</h3>
              <p className="text-muted-foreground mb-6">
                Choose a payment gateway from the list above to configure its settings
              </p>
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setGlobalSettings(prev => ({ ...prev, sessionTimeout: parseInt(e.target.value) }))}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setGlobalSettings(prev => ({ ...prev, minimumAmount: parseFloat(e.target.value) }))}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setGlobalSettings(prev => ({ ...prev, maximumAmount: parseFloat(e.target.value) }))}
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Enable test mode for all gateways</p>
                  </div>
                  <Switch
                    id="enable-test-mode"
                    checked={globalSettings.enableTestMode}
                    onCheckedChange={(checked) => setGlobalSettings(prev => ({ ...prev, enableTestMode: checked }))}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Allow automatic refund processing</p>
                  </div>
                  <Switch
                    id="enable-refunds"
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <Label htmlFor="enable-fraud-detection">Fraud Detection</Label>
                    <p className="text-sm text-muted-foreground">Enable automatic fraud prevention</p>
                  </div>
                  <Switch
                    id="enable-fraud-detection"
                    checked={globalSettings.enableFraudDetection}
                    onCheckedChange={(checked) => setGlobalSettings(prev => ({ ...prev, enableFraudDetection: checked }))}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <Label htmlFor="enable-save-cards">Save Card Details</Label>
                    <p className="text-sm text-muted-foreground">Allow users to save cards for future payments</p>
                  </div>
                  <Switch
                    id="enable-save-cards"
                    checked={globalSettings.enableSaveCards}
                    onCheckedChange={(checked) => setGlobalSettings(prev => ({ ...prev, enableSaveCards: checked }))}
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => setGlobalSettings(prev => ({ ...prev, refundPolicy: e.target.value }))}
                  rows={3}
              ...
              <Alert>
                <Shield className="h-4 w-4" />
                <AlertDescription>
                  <strong>Security Notice:</strong> All payment data is encrypted and processed securely. We never store sensitive payment information on our servers.
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
