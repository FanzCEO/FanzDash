...
export default function SocialLoginSettings() {
  ...
  const handleProviderUpdate = (id: string, field: keyof SocialProvider, value: any) => {
  ...
  const getProviderColor = (provider: string) => {
    switch (provider) {
      ...
      case 'google': return 'bg-red-50 border-red-200';
  ...
  return (
    <div className="container mx-auto p-6 space-y-6" data-testid="social-login-settings">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold cyber-text-glow">Social Login Settings</h1>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{getEnabledProvidersCount()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{getTotalConnections().toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          return (
            <Card key={provider.id} className={`cyber-border ${getProviderColor(provider.provider)}`}>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <CardTitle>{getProviderName(provider.provider)} Login</CardTitle>
                      <CardDescription>
                        OAuth integration for {getProviderName(provider.provider)} authentication
                      </CardDescription>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor={`${provider.provider}-client-id`}>
                      {provider.provider === 'facebook' ? 'App ID' : 
                       provider.provider === 'twitter' ? 'API Key' : 'Client ID'}
                    </Label>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor={`${provider.provider}-client-secret`}>
                      {provider.provider === 'facebook' ? 'App Secret' : 
                       provider.provider === 'twitter' ? 'API Secret' : 'Client Secret'}
                    </Label>
                    <div className="relative">
                      <Input
                ...
                {/* Callback URL */}
                <div className="space-y-2">
                  <Label htmlFor={`${provider.provider}-callback`}>Callback URL</Label>
                  <div className="relative">
                    <Input
                  ...
                  <p className="text-sm text-muted-foreground">
                    Add this URL to your {getProviderName(provider.provider)} app's authorized redirect URIs
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t">
                  <div className="flex items-center space-x-4">
                    <Button
                      variant="outline"
                      onClick={() => testConnectionMutation.mutate(provider.provider)}
                      disabled={!provider.clientId || !provider.clientSecret || testConnectionMutation.isPending}
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <ul className="text-sm text-yellow-700 mt-2 space-y-1">
                ...
                <li>• Keep your OAuth scopes minimal and only request necessary permissions</li>
              </ul>
            </div>
          </div>
        </CardContent>
