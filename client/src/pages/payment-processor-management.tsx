...
import { AlertTriangle, Check, X, Plus, Settings, CreditCard, Bitcoin, Globe } from "lucide-react";
...
export default function PaymentProcessorManagement() {
  ...
  const processors = [
    {
      id: '1', name: 'CCBill', slug: 'ccbill', processorType: 'adult_friendly' as const,
      region: 'Global', status: 'active' as const, isBanned: false, adultFriendly: true,
      supportedCurrencies: ['USD', 'EUR', 'GBP'], processingTime: '1-3 days', testMode: false
    },
    {
      id: '2', name: 'Segpay', slug: 'segpay', processorType: 'adult_friendly' as const,
      region: 'Global', status: 'active' as const, isBanned: false, adultFriendly: true,
      supportedCurrencies: ['USD', 'EUR'], processingTime: '2-5 days', testMode: false
    },
    {
      id: '3', name: 'Epoch', slug: 'epoch', processorType: 'adult_friendly' as const,
      region: 'Global', status: 'active' as const, isBanned: false, adultFriendly: true,
      supportedCurrencies: ['USD', 'EUR', 'CAD'], processingTime: '1-2 days', testMode: false
    },
    {
      id: '4', name: 'Binance', slug: 'binance', processorType: 'crypto' as const,
      region: 'Global', status: 'active' as const, isBanned: false, adultFriendly: true,
      supportedCurrencies: ['BTC', 'ETH', 'BNB', 'USDT'], processingTime: 'instant', testMode: false
    },
    {
      id: '5', name: 'Coinbase', slug: 'coinbase', processorType: 'crypto' as const,
      region: 'Global', status: 'active' as const, isBanned: false, adultFriendly: true,
      supportedCurrencies: ['BTC', 'ETH', 'USDC'], processingTime: 'instant', testMode: false
    },
    {
      id: '6', name: 'Flutterwave', slug: 'flutterwave', processorType: 'regional' as const,
      region: 'Africa', status: 'active' as const, isBanned: false, adultFriendly: true,
      supportedCurrencies: ['USD', 'NGN', 'KES'], processingTime: '1-3 days', testMode: false
    },
    {
  ...
  const updateProcessorMutation = useMutation({
    mutationFn: (data: { id: string, updates: Partial<PaymentProcessor> }) =>
      apiRequest(`/api/admin/payment-processors/${data.id}`, 'PATCH', data.updates),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/admin/payment-processors'] });
      toast({ title: "Payment processor updated successfully" });
    }
  });

  const bannedProcessors = [
    { name: "Stripe", reason: "Adult content policy violation", type: "traditional" },
    { name: "PayPal", reason: "Adult content policy violation", type: "traditional" },
    { name: "Square", reason: "Adult content restrictions", type: "traditional" },
    { name: "Venmo", reason: "Adult content policy violation", type: "traditional" }
  ];

  const adultFriendlyProcessors = processors.filter(p => p.adultFriendly && !p.isBanned);
  const cryptoProcessors = processors.filter(p => p.processorType === 'crypto');
  const regionalProcessors = processors.filter(p => p.processorType === 'regional');
  ...
  const getStatusBadge = (processor: PaymentProcessor) => {
    ...
    if (processor.status === 'active') return <Badge variant="default">Active</Badge>;
    if (processor.status === 'inactive') return <Badge variant="secondary">Inactive</Badge>;
    return <Badge variant="outline">{processor.status}</Badge>;
  };

  return (
    <div className="container mx-auto p-6 space-y-6" data-testid="payment-processor-management">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Payment Processor Management</h1>
          <p className="text-muted-foreground">
            Manage adult-friendly payment processors and enforce compliance policies
          </p>
        </div>
      ...
      <Card className="border-red-200 bg-red-50 dark:bg-red-950/10">
        <CardHeader>
          ...
          <CardDescription className="text-red-600 dark:text-red-400">
            These processors are automatically blocked due to adult content policies
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Tabs defaultValue="adult-friendly" className="w-full">
        <TabsList className="grid w-full grid-cols-4">
          <TabsTrigger value="adult-friendly">Adult-Friendly ({adultFriendlyProcessors.length})</TabsTrigger>
          <TabsTrigger value="crypto">Crypto ({cryptoProcessors.length})</TabsTrigger>
          <TabsTrigger value="regional">Regional ({regionalProcessors.length})</TabsTrigger>
          <TabsTrigger value="all">All Processors ({processors.length})</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="adult-friendly" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Payment processors that support adult content and creator economy platforms
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {adultFriendlyProcessors.map((processor: PaymentProcessor) => (
                    <TableRow key={processor.id}>
                      <TableCell className="flex items-center">
                        {getProcessorIcon(processor.processorType)}
                        <span className="ml-2 font-medium">{processor.name}</span>
                      </TableCell>
                      <TableCell>
        ...
        <TabsContent value="crypto" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Crypto payment gateways for anonymous and decentralized transactions
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <TableBody>
                  {processors.map((processor: PaymentProcessor) => (
                    <TableRow key={processor.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline">{processor.processorType}</Badge>
                      </TableCell>
                      <TableCell>
