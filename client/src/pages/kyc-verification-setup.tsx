...
export default function KYCVerificationSetup() {
  ...
  const [kycLevels] = useState<KYCLevel[]>([
    ...
    },
    {
      ...
      description: 'Full documentation for business accounts and high-value transactions',
      requirements: ['All Standard Requirements', 'Proof of Income', 'Business Registration (if applicable)'],
      limits: {
        daily: 10000,
        monthly: 100000,
        withdrawal: 25000
      },
      documents: ['Government ID', 'Proof of Address', 'Proof of Income', 'Business Documents'],
  ...
  const [kycTemplates] = useState<KYCTemplate[]>([
    {
      ...
      fields: [
        ...
        { id: 'nationality', name: 'Nationality', type: 'select', required: true, options: ['US', 'UK', 'CA', 'AU', 'DE', 'FR'] },
        { id: 'address', name: 'Address', type: 'address', required: true },
        { id: 'occupation', name: 'Occupation', type: 'text', required: false },
        { id: 'sourceOfFunds', name: 'Source of Funds', type: 'select', required: true, options: ['Salary', 'Business', 'Investment', 'Other'] }
      ],
      documents: [
        { id: 'governmentId', name: 'Government ID', type: 'identity', required: true, description: 'Passport, Driver License, or National ID' },
        { id: 'proofOfAddress', name: 'Proof of Address', type: 'address', required: true, description: 'Utility bill or bank statement (last 3 months)' },
        { id: 'selfie', name: 'Selfie Photo', type: 'identity', required: true, description: 'Clear photo of yourself holding your ID' }
      ]
    },
    {
      id: 'business',
      ...
      fields: [
        ...
        { id: 'registrationNumber', name: 'Registration Number', type: 'text', required: true },
        { id: 'businessType', name: 'Business Type', type: 'select', required: true, options: ['Corporation', 'LLC', 'Partnership', 'Sole Proprietorship'] },
        { id: 'incorporationDate', name: 'Incorporation Date', type: 'date', required: true },
        { id: 'businessAddress', name: 'Business Address', type: 'address', required: true },
        { id: 'website', name: 'Company Website', type: 'text', required: false },
        { id: 'annualRevenue', name: 'Annual Revenue', type: 'select', required: true, options: ['<$100K', '$100K-$1M', '$1M-$10M', '>$10M'] }
      ],
      documents: [
        { id: 'businessRegistration', name: 'Business Registration', type: 'business', required: true, description: 'Certificate of incorporation or registration' },
        { id: 'businessLicense', name: 'Business License', type: 'business', required: true, description: 'Valid business license or permit' },
        { id: 'taxCertificate', name: 'Tax Certificate', type: 'business', required: true, description: 'Tax registration certificate' },
        { id: 'bankStatement', name: 'Bank Statement', type: 'business', required: true, description: 'Business bank statement (last 3 months)' }
      ]
    }
  ]);
  ...
  const getComplianceLevelBadge = (level: string) => {
    switch (level) {
      ...
      case 'enhanced':
        return <Badge className="bg-purple-100 text-purple-800">Enhanced</Badge>;
      default:
  ...
  const LevelCard = ({ level }: { level: KYCLevel }) => (
    <Card className={`hover:shadow-lg transition-all duration-300 ${!level.isEnabled ? 'opacity-60' : ''}`}>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              <div>
                ...
                <div className="font-semibold">${level.limits.daily.toLocaleString()}</div>
              </div>
              <div>
                <div className="text-muted-foreground">Monthly Limit</div>
                <div className="font-semibold">${level.limits.monthly.toLocaleString()}</div>
              </div>
              <div>
                <div className="text-muted-foreground">Withdrawal Limit</div>
                <div className="font-semibold">${level.limits.withdrawal.toLocaleString()}</div>
              </div>
              <div>
    ...
    <Card className="hover:shadow-lg transition-all duration-300">
      <CardContent className="p-6">
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <div>
              <h4 className="font-medium text-sm mb-2">Form Fields ({template.fields.length})</h4>
              <div className="flex flex-wrap gap-1">
            ...
            <div>
              <h4 className="font-medium text-sm mb-2">Documents ({template.documents.length})</h4>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Configure know-your-customer verification requirements and compliance settings
          </p>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-blue-600">{complianceStats.totalSubmissions}</div>
            ...
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-purple-600">{complianceStats.complianceRate}%</div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={kycSettings.enableKyc ? "default" : "secondary"}>
                      {kycSettings.enableKyc ? <CheckCircle className="h-3 w-3 mr-1" /> : <XCircle className="h-3 w-3 mr-1" />}
                      {kycSettings.enableKyc ? "Active" : "Disabled"}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Auto Approval</span>
                    <Badge variant={kycSettings.autoApproval ? "default" : "outline"}>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={kycSettings.requireForWithdraw ? "default" : "outline"}>
                      {kycSettings.requireForWithdraw ? "Required" : "Optional"}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Liveness Detection</span>
                    <Badge variant={kycSettings.enableLiveness ? "default" : "secondary"}>
                      {kycSettings.enableLiveness ? "Enabled" : "Disabled"}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Average Processing Time</span>
                    <Badge variant="outline">{complianceStats.avgProcessingTime}</Badge>
                  </div>
                </div>
              </div>
              ...
              <Alert>
                ...
                <AlertDescription>
                  <strong>Compliance Notice:</strong> KYC verification is required by law for financial services. Ensure your settings comply with local regulations.
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <Label htmlFor="enable-kyc">Enable KYC Verification</Label>
                    <p className="text-sm text-muted-foreground">Require identity verification for platform access</p>
                  </div>
                  <Switch
                    id="enable-kyc"
                    checked={kycSettings.enableKyc}
                    onCheckedChange={(checked) => updateSetting('enableKyc', checked)}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Automatically approve low-risk submissions</p>
                  </div>
                  <Switch
                    id="auto-approval"
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <Label htmlFor="require-withdraw">Required for Withdrawals</Label>
                    <p className="text-sm text-muted-foreground">Require KYC before allowing withdrawals</p>
                  </div>
                  <Switch
                    id="require-withdraw"
                    checked={kycSettings.requireForWithdraw}
                    onCheckedChange={(checked) => updateSetting('requireForWithdraw', checked)}
                    disabled={!kycSettings.enableKyc}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <Label htmlFor="require-subscription">Required for Subscriptions</Label>
                    <p className="text-sm text-muted-foreground">Require KYC before subscribing to creators</p>
                  </div>
                  <Switch
                    id="require-subscription"
                    checked={kycSettings.requireForSubscription}
                    onCheckedChange={(checked) => updateSetting('requireForSubscription', checked)}
                    disabled={!kycSettings.enableKyc}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value: any) => updateSetting('complianceLevel', value)}
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="standard">Standard - ID Verification</SelectItem>
                      <SelectItem value="enhanced">Enhanced - Full Documentation</SelectItem>
                    </SelectContent>
                  </Select>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="max-attempts">Max Verification Attempts</Label>
                  <Input
                    id="max-attempts"
                    type="number"
                    min="1"
                    ...
                    onChange={(e) => updateSetting('maxVerificationAttempts', parseInt(e.target.value))}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => updateSetting('verificationTimeout', parseInt(e.target.value))}
                    data-testid="verification-timeout-input"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="document-expiry">Document Validity (days)</Label>
                  <Input
                    id="document-expiry"
                    type="number"
                    min="30"
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Verify selfies are taken by live persons</p>
                </div>
                <Switch
                  id="enable-liveness"
                  checked={kycSettings.enableLiveness}
                  onCheckedChange={(checked) => updateSetting('enableLiveness', checked)}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Perform automated risk scoring</p>
                </div>
                <Switch
                  id="risk-assessment"
                  checked={kycSettings.riskAssessment}
                  onCheckedChange={(checked) => updateSetting('riskAssessment', checked)}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="sanctions-screening">Sanctions Screening</Label>
                  <p className="text-sm text-muted-foreground">Check against international sanctions lists</p>
                </div>
                <Switch
                  id="sanctions-screening"
                  checked={kycSettings.sanctions_screening}
                  onCheckedChange={(checked) => updateSetting('sanctions_screening', checked)}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Screen for Politically Exposed Persons</p>
                </div>
                <Switch
                  id="pep-screening"
                  checked={kycSettings.pep_screening}
                  onCheckedChange={(checked) => updateSetting('pep_screening', checked)}
              ...
              <Alert>
                ...
                <AlertDescription>
                  Advanced security features may require additional API integrations and may increase verification processing time.
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
        ...
        <TabsContent value="compliance" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="font-semibold mb-3">Supported Regulations</h4>
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <CheckCircle className="h-4 w-4 text-green-600" />
                      <span className="text-sm">AML (Anti-Money Laundering)</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <CheckCircle className="h-4 w-4 text-green-600" />
                      <span className="text-sm">KYC (Know Your Customer)</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <CheckCircle className="h-4 w-4 text-green-600" />
              ...
              <Alert>
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>
                  <strong>Compliance Warning:</strong> Ensure your KYC settings comply with regulations in your jurisdiction. Consult with legal experts for specific requirements.
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="text-sm text-muted-foreground">
                Reports include verification statistics, compliance metrics, and audit trails for regulatory review.
              </div>
            </CardContent>
          </Card>
        </TabsContent>
