    ...
    <div className="min-h-screen cyber-bg">
      <div className="container mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold cyber-text-glow">Threat Center</h1>
            <p className="text-muted-foreground">Real-time Security Monitoring & Threat Intelligence</p>
          </div>
          <Button 
        ...
        <Card className={`cyber-card border-2 ${getThreatColor(threatIntelligence.currentLevel)}`}>
          <CardHeader>
            <CardTitle className="flex items-center space-x-2">
              <AlertTriangle className="w-8 h-8 text-orange-500 cyber-pulse" />
              <span className="text-2xl cyber-text-glow">THREAT LEVEL: {threatIntelligence.currentLevel}</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
          ...
          <CardContent>
            <div className="space-y-4 max-h-80 overflow-y-auto">
              {threatData?.map((alert: any) => (
                <div key={alert.id} className={`p-4 cyber-card border-2 ${getThreatColor(alert.severity)}`}>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-sm">
                      <span className="text-muted-foreground">Affected Platforms: </span>
                      <span className="text-primary">{alert.affectedPlatforms?.join(', ')}</span>
                    </div>
                    <Badge variant="outline" className={alert.status === 'ACTIVE' ? 'text-red-400' : 'text-yellow-400'}>
                      {alert.status}
                    </Badge>
                  </div>
                  ...
                  <div className="mt-3">
                    ...
                    <ul className="list-disc list-inside text-sm space-y-1">
                      {alert.recommendedActions?.map((action: string, index: number) => (
                        <li key={index} className="text-primary">{action}</li>
                      ))}
                    </ul>
                  </div>
                </div>
            ...
            <CardContent>
              <div className="space-y-4">
                {threatIntelligence.trends.map((trend, index) => (
                  <div key={index} className="flex items-center justify-between p-3 cyber-card border border-primary/20">
                    <div>
                    ...
                    <div className="text-right">
                      ...
                      <div className={`w-2 h-2 rounded-full ${
                        ...
                        trend.trend === 'decreasing' ? 'bg-green-500' : 'bg-yellow-500'
                      } cyber-pulse`} />
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-4">
                {threatIntelligence.topTargets.map((target, index) => (
                  <div key={index} className="flex items-center justify-between p-3 cyber-card border border-primary/20">
                    ...
                    <Badge 
                      variant={target.severity === 'HIGH' ? 'destructive' : 
                              target.severity === 'MEDIUM' ? 'default' : 'secondary'}
                    >
                      {target.severity}
                    </Badge>
                  </div>
        ...
        <Card className="cyber-card">
          <CardHeader>
            <CardTitle className="flex items-center space-x-2">
              ...
              <span className="cyber-text-glow">SECURITY OPERATIONS STATUS</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="text-center p-4 cyber-card border border-blue-500/30">
                ...
                <div className="text-xl font-bold text-blue-400">OPERATIONAL</div>
                <div className="text-sm text-muted-foreground">Defense Systems</div>
              </div>
