...
import { MultiAuthService, authRateLimit, strictAuthRateLimit, validateRegistration, validateLogin, validateFanzId, validateTOTP, authenticateToken } from './multiAuth';
  ...
  passport.authenticate(provider, { session: false }, async (err: any, user: any) => {
    ...
    try {
      ...
      if (securityResult.requiresVerification) {
        ...
        return res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:3000'}/auth/device-verification?pending=true`);
      }
      ...
      res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:3000'}/auth/login?error=callback_failed`);
    }
  })(req, res, next);
});

// Local Authentication Routes
router.post('/auth/login', strictAuthRateLimit, validateLogin, async (req, res) => {
  ...
  passport.authenticate('local', { session: false }, async (err: any, user: any, info: any) => {
    ...
    try {
      ...
      if (requiresMFA) {
        ...
        const tempToken = MultiAuthService.generateJWT({ ...user, temp: true });
        return res.json({
          success: false,
          requiresMFA: true,
          tempToken,
...
router.post('/auth/register', authRateLimit, validateRegistration, async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array() });
  }
...
// FanzID Management
router.post('/auth/create-fanz-id', authenticateToken, validateFanzId, async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array() });
  }
...
router.post('/auth/verify-2fa', authenticateToken, validateTOTP, async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array() });
  }
...
router.delete('/auth/trusted-device/:fingerprint', authenticateToken, async (req, res) => {
  try {
    const { fingerprint } = req.params;
    const userId = req.user.id;

    const success = await DeviceSecurityService.removeTrustedDevice(userId, fingerprint);
