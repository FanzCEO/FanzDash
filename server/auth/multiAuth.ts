...
import { db } from '../db';
import { users, loginSessions, oauthStates, webauthnCredentials, securityAuditLog, trustedDevices, emailVerificationTokens } from '@shared/schema';
...
export class MultiAuthService {
  ...
  private static handleOAuthCallback(provider: string) {
    return async (accessToken: string, refreshToken: string, profile: any, done: any) => {
      try {
        const providerIdField = `${provider}Id` as keyof typeof users.$inferSelect;
        const email = profile.emails?.[0]?.value || null;
        
        // Check if user exists with this provider ID
        let user = await db.select().from(users).where(
          eq(users[providerIdField] as any, profile.id)
        ).limit(1);
        ...
        await this.logSecurityEvent(user[0].id, 'oauth_login', { provider, success: true });
        done(null, user[0]);
      } catch (error) {
  ...
  private static handleLocalAuth = async (identifier: string, password: string, done: any) => {
    try {
      ...
      if (!foundUser) {
        ...
        const userByFanzId = await db.select().from(users).where(
          and(
            eq(users.fanzId, identifier),
            eq(users.fanzIdEnabled, true)
          )
        ).limit(1);
  ...
  static async createFanzId(userId: string, fanzId: string): Promise<AuthResult> {
    try {
      // Check if FanzID is already taken
      const existing = await db.select().from(users).where(eq(users.fanzId, fanzId)).limit(1);
      ...
      await this.logSecurityEvent(userId, 'fanz_id_created', { fanzId, success: true });
  ...
  static async verifyAndEnableTOTP(userId: string, token: string): Promise<AuthResult> {
    try {
      const user = await this.getUserById(userId);
  ...
  static async verifyTOTP(userId: string, token: string): Promise<boolean> {
    try {
      ...
      if (user.backupCodes?.includes(token)) {
        ...
        const newBackupCodes = user.backupCodes.filter((code: string) => code !== token);
        await db.update(users)
          .set({ backupCodes: newBackupCodes })
          .where(eq(users.id, userId));
        
        await this.logSecurityEvent(userId, 'backup_code_used', { success: true });
        return true;
      }
