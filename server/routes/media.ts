...
const validateSyncRequest = [
  ...
  body('targetPlatforms.*').isIn(['boyfanz', 'girlfanz', 'pupfanz', 'transfanz', 'taboofanz', 'fanztube', 'fanzcock']).withMessage('Invalid platform'),
...
router.get('/platforms/status', generalLimiter, requireAuth, async (req, res) => {
  try {
    ...
    const platformStatus = {
      ...
      fanzcock: { online: true, lastSync: new Date(), uploadQueue: 2 }
    };
