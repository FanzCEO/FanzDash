...
const validateAccessCheck = [
  ...
  body('platform').isIn(['BoyFanz', 'GirlFanz', 'PupFanz', 'TransFanz', 'TabooFanz', 'FanzTube', 'FanzCock']).withMessage('Invalid platform'),
...
router.get('/platforms/status', generalLimiter, async (req, res) => {
  try {
    ...
    const platformStatus = {
      ...
      FanzCock: { online: true, users: 1800, lastSync: new Date() }
    };
...
router.get('/domains', generalLimiter, (req, res) => {
  try {
    ...
    const domainMappings = {
      ...
      'fanzcock.com': { platform: 'FanzCock', requiresAuth: true, tier: 'premium' }
    };
