...
export class Compliance2257Service {
  ...
  async createRecord(userId: string, recordData: Partial<InsertForm2257Record>, metadata: any): Promise<Form2257Record> {
    try {
  ...
  async updateRecord(recordId: string, updates: Partial<InsertForm2257Record>, amendedBy: string): Promise<Form2257Record> {
    try {
      ...
      await this.createAmendment(recordId, 'update', currentRecord, updates, 'Record update', amendedBy);

      // Update the record
      const [updatedRecord] = await db.update(form2257Records)
  ...
  verifyAge(dateOfBirth: string, minimumAge: number = 18): { isValid: boolean; age: number; details: string } {
    try {
  ...
  validateIdDocument(idType: string, idNumber: string, issuer: string, issueDate: string, expirationDate?: string): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    // Validate ID types
    const validIdTypes = ['driver_license', 'passport', 'state_id', 'military_id'];
  ...
  createDigitalSignature(data: string, metadata: any): DigitalSignature {
    ...
    const signature = crypto.createHash('sha256').update(hash + metadata.userId).digest('hex');
  ...
  async verifyCompliance(recordId: string, verifiedBy: string): Promise<{ isCompliant: boolean; issues: string[]; score: number }> {
    try {
      ...
      const requiredFields = [
        ...
        'primaryIdType', 'primaryIdNumber', 'primaryIdIssuer',
        'performanceDate', 'custodianName', 'custodianTitle', 'custodianAddress'
      ];
      ...
      await db.update(form2257Records)
        .set({
  ...
  async searchRecords(filters: {
    ...
    try {
      ...
      if (filters.verificationStatus) {
        conditions.push(eq(form2257Records.verificationStatus, filters.verificationStatus));
      }

      if (filters.performanceDateFrom) {
        conditions.push(gte(form2257Records.performanceDate, filters.performanceDateFrom));
      }

      if (filters.performanceDateTo) {
        conditions.push(lte(form2257Records.performanceDate, filters.performanceDateTo));
      }
  ...
  private async createComplianceChecklist(recordId: string, checkedBy: string): Promise<void> {
    try {
  ...
  // Update compliance checklist
  private async updateComplianceChecklist(recordId: string, checkedBy: string): Promise<void> {
    try {
  ...
  async getComplianceStats(): Promise<{
    ...
    try {
      ...
      stats.complianceRate = stats.totalRecords > 0 
        ? Math.round((stats.approved / (stats.totalRecords - stats.expired)) * 100)
        : 0;
  ...
  private async logComplianceEvent(userId: string, event: string, details: any): Promise<void> {
    try {
  ...
  async cleanupExpiredRecords(): Promise<number> {
    try {
      ...
      console.log(`Found ${expiredRecords.length} expired records for archival`);
