...
export class CDNDistribution extends EventEmitter {
    ...
    const defaultNodes: CDNNode[] = [
      {
        ...
        performance: { latency: 45, uptime: 99.9, errorRate: 0.01, throughput: 8500 },
        priority: 9,
        costPerGB: 0.05,
        supportedMimeTypes: ['*/*'],
      ...
      },
      {
        ...
        performance: { latency: 42, uptime: 99.8, errorRate: 0.02, throughput: 7200 },
        priority: 8,
        costPerGB: 0.06,
        supportedMimeTypes: ['*/*'],
        features: ['compression', 'streaming', 'ssl', 'ipv6']
      },
      {
        ...
        performance: { latency: 38, uptime: 99.7, errorRate: 0.03, throughput: 5400 },
        priority: 7,
        costPerGB: 0.07,
        supportedMimeTypes: ['*/*'],
        features: ['compression', 'streaming', 'ssl', 'ipv6']
      },
      {
        ...
        performance: { latency: 65, uptime: 99.6, errorRate: 0.04, throughput: 4200 },
        priority: 6,
        costPerGB: 0.08,
        supportedMimeTypes: ['*/*'],
      ...
      },
      {
        ...
        performance: { latency: 25, uptime: 99.9, errorRate: 0.01, throughput: 12000 },
        priority: 10,
        costPerGB: 0.12,
        supportedMimeTypes: ['image/*', 'video/*', 'application/javascript', 'text/css'],
        features: ['compression', 'mobile-optimization', 'ssl', 'ipv6', 'http3']
      }
    ];
  ...
  async addAsset(
    ...
    const existingAsset = Array.from(this.assets.values())
      .find(asset => asset.checksum === checksum);
    ...
    const selectedNodes = this.selectOptimalNodes(asset, options.preferredNodes);
  ...
  private async extractAssetMetadata(filepath: string, mimeType: string): Promise<CDNAsset['metadata']> {
  ...
  private async generateAssetVariants(asset: CDNAsset): Promise<void> {
    ...
    if (asset.mimeType.startsWith('image/')) {
      ...
      const sizes = [
        ...
        { name: 'webp', width: asset.metadata.width, height: asset.metadata.height, quality: 85, format: 'webp' },
        { name: 'avif', width: asset.metadata.width, height: asset.metadata.height, quality: 80, format: 'avif' }
      ];

      for (const size of sizes) {
        try {
          const variantPath = join(process.cwd(), 'media', 'variants', `${asset.id}_${size.name}.${size.format || 'jpg'}`);
          
          const command = [
            '-i', asset.originalPath,
            '-vf', `scale=${size.width}:${size.height}:force_original_aspect_ratio=decrease`,
            '-q:v', size.quality.toString(),
            '-y', variantPath
          ];
      ...
      try {
        const thumbnailPath = join(process.cwd(), 'media', 'variants', `${asset.id}_thumbnail.jpg`);
      ...
      for (const quality of qualities) {
        if (asset.metadata.height && asset.metadata.height < quality.height) continue;

        try {
          const variantPath = join(process.cwd(), 'media', 'variants', `${asset.id}_${quality.name}.mp4`);
  ...
  private selectOptimalNodes(asset: CDNAsset, preferredNodes?: string[]): string[] {
    ...
    if (preferredNodes && preferredNodes.length > 0) {
      const preferred = availableNodes.filter(node => preferredNodes.includes(node.id));
      const others = availableNodes.filter(node => !preferredNodes.includes(node.id));
  ...
  private async distributeAsset(asset: CDNAsset): Promise<void> {
    ...
    for (const nodeId of asset.distribution.nodes) {
      try {
        ...
        console.error(`Failed to upload asset ${asset.id} to node ${nodeId}:`, error);
      }
    }
  ...
  private async uploadAssetToNode(asset: CDNAsset, nodeId: string): Promise<void> {
  ...
  async getAssetURL(assetId: string, clientIP?: string, quality?: string): Promise<string> {
    ...
    if (quality && quality !== 'original') {
      const variant = asset.variants.find(v => v.quality === quality || v.type === quality);
      if (variant) {
  ...
  private selectBestNodeForClient(asset: CDNAsset, clientIP?: string): CDNNode | null {
    const availableNodes = asset.distribution.nodes
      .map(nodeId => this.nodes.get(nodeId))
      .filter((node): node is CDNNode => node !== undefined && node.status === 'active');
    ...
    if (clientIP) {
      ...
      availableNodes.sort((a, b) => a.performance.latency - b.performance.latency);
    }
  ...
  private getCompressionType(mimeType: string): CDNAsset['caching']['compression'] {
  ...
  private updateNodeMetrics(): void {
    for (const node of this.nodes.values()) {
      ...
      node.performance.uptime = Math.max(95, baseUptime + (Math.random() - 0.5) * 2);
      
      node.performance.errorRate = Math.max(0, Math.random() * 0.1);
      node.performance.throughput = Math.max(0, node.usage.bandwidth + (Math.random() - 0.5) * 1000);
  ...
  private updateAssetHotness(): void {
    ...
    for (const asset of this.assets.values()) {
      const hoursSinceAccess = (now.getTime() - asset.lastAccessed.getTime()) / (1000 * 60 * 60);
      const accessFrequency = asset.accessCount / Math.max(1, hoursSinceAccess);
      
      // Calculate hotness score (0-100)
      const hotnessScore = Math.min(100, Math.log10(1 + accessFrequency * 10) * 25);
  ...
  private optimizeDistribution(): void {
    ...
    for (const asset of hotAssets.slice(0, 10)) { // Top 10 hot assets
      ...
      if (currentNodes < targetNodes) {
        ...
        const additionalNodes = this.selectOptimalNodes(asset, asset.distribution.nodes);
        const newNodes = additionalNodes.filter(nodeId => 
          !asset.distribution.nodes.includes(nodeId)
  ...
  private syncAssets(): void {
    ...
    const assetsToSync = Array.from(this.assets.values())
      .filter(asset => asset.distribution.syncStatus === 'failed' || 
  ...
  getStatistics() {
    ...
    return {
      ...
      nodes: {
        ...
        averageLatency: Math.round(nodes.reduce((sum, n) => sum + n.performance.latency, 0) / nodes.length),
        averageUptime: Math.round(nodes.reduce((sum, n) => sum + n.performance.uptime, 0) / nodes.length * 100) / 100
      },
      requests: {
        total: totalRequests,
        hitRate: Math.round(hitRate * 10000) / 100, // Percentage with 2 decimals
        totalBandwidth: Math.round(requests.reduce((sum, r) => sum + r.bytesTransferred, 0) / (1024 * 1024 * 1024) * 100) / 100
      },
      performance: {
        ...
        errorRate: requests.length > 0
          ? Math.round(requests.filter(r => r.status >= 400).length / requests.length * 10000) / 100
          : 0
      }
    };
  }
