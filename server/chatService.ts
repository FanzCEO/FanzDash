...
export class ChatService {
  ...
  private async handleJoinRoom(clientId: string, message: any) {
    try {
      ...
      const canJoin = await this.canUserJoinRoom(client.userId, roomId, roomType);
      if (!canJoin) {
  ...
  private async saveMessage(messageData: InsertChatMessage): Promise<ChatMessage> {
    const [message] = await db.insert(chatMessages).values(messageData).returning();
    return message;
  }

  private async getChatHistory(roomId: string, limit: number, offset: number): Promise<any[]> {
    try {
  ...
  private async moderateContent(content: string): Promise<boolean> {
    try {
      if (!process.env.OPENAI_API_KEY) {
        ...
        const toxicKeywords = ['hate', 'harassment', 'violence', 'illegal', 'spam'];
  ...
  async createChatRoom(creatorId: string, name: string, description?: string, isPrivate: boolean = false): Promise<ChatRoom> {
  ...
  getConnectedUsers(roomId?: string): any[] {
    if (roomId) {
      ...
      return Array.from(roomClients).map(clientId => {
        const client = this.clients.get(clientId);
        return client ? {
          userId: client.userId,
          username: client.username,
