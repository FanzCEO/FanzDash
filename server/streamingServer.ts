...
export class StreamingServer extends EventEmitter {
  ...
  private startRTMPServer() {
    ...
    this.rtmpServer = spawn('ffmpeg', [
      '-listen', '1',
      '-f', 'flv',
      '-i', `rtmp://localhost:${this.RTMP_PORT}/live`,
      '-c', 'copy',
      '-f', 'null',
      '-'
    ]);
  ...
  private async setupStreamProcessing(session: StreamSession) {
    ...
    const dashPath = join(process.cwd(), 'media', 'streams', 'dash', session.id);
    const recordingPath = join(process.cwd(), 'media', 'streams', 'recordings', `${session.id}.mp4`);
    ...
    const ffmpegArgs = [
      '-f', 'flv',
      '-listen', '1',
      '-i', `rtmp://localhost:${this.RTMP_PORT}/live/${session.streamKey}`,
      
      // Video encoding
      '-c:v', session.settings.videoCodec === 'h265' ? 'libx265' : 'libx264',
      '-preset', 'veryfast',
      '-tune', 'zerolatency',
      '-maxrate', `${session.settings.maxBitrate}k`,
      '-bufsize', `${session.settings.maxBitrate * 2}k`,
      '-vf', `scale=${session.settings.resolution.replace('x', ':')}`,
      '-r', session.settings.fps.toString(),
      '-g', (session.settings.fps * 2).toString(), // Keyframe interval
      
      // Audio encoding
      '-c:a', session.settings.audioCodec,
      '-b:a', '128k',
      '-ar', '48000',
      '-ac', '2',
      ...
      '-hls_segment_filename', join(hlsPath, 'segment_%03d.ts'),
    ...
    streamProcess.stderr?.on('data', (data) => {
      ...
      if (session.settings.enableThumbnails && Math.random() < 0.01) { // 1% chance per frame
  ...
  private parseStreamInfo(session: StreamSession, output: string) {
    ...
    if (timeMatch) {
      ...
      session.duration = parseInt(hours) * 3600 + parseInt(minutes) * 60 + parseFloat(seconds);
    }
  ...
  private async generateStreamThumbnail(session: StreamSession) {
    const thumbnailPath = join(process.cwd(), 'media', 'streams', 'thumbnails', `${session.id}_${Date.now()}.jpg`);
  ...
  private async handleHttpRequest(req: any, res: any) {
    ...
    try {
      ...
      if (pathParts[0] === 'hls' && pathParts.length >= 2) {
        ...
        const filePath = join(process.cwd(), 'media', 'streams', 'hls', sessionId, filename);
      ...
      if (pathParts[0] === 'dash' && pathParts.length >= 2) {
        ...
        const filePath = join(process.cwd(), 'media', 'streams', 'dash', sessionId, filename);
      ...
      if (pathParts[0] === 'thumbnails' && pathParts.length === 2) {
        ...
        const filePath = join(process.cwd(), 'media', 'streams', 'thumbnails', filename);
  ...
  private handleWebSocketMessage(ws: WebSocket, viewerId: string, message: any) {
    switch (message.type) {
      case 'joinStream':
      ...
      case 'sendChat':
        this.handleChatMessage(message.sessionId, message.userId, message.username, message.message);
        break;
  ...
  private addViewer(viewerId: string, sessionId: string, ws: WebSocket, userId?: string) {
  ...
  private removeAllViewers(sessionId: string) {
    const viewersToRemove = Array.from(this.viewers.values())
  ...
  private handleChatMessage(sessionId: string, userId: string | undefined, username: string, message: string) {
    const session = this.sessions.get(sessionId);
  ...
  getStats() {
    ...
    return {
      ...
      avgViewersPerStream: sessions.length > 0 
        ? Math.round(this.viewers.size / sessions.filter(s => s.status === 'live').length) || 0
        : 0
    };
  }
}
